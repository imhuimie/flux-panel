...
export default function IndexPage() {
  ...
  const initCaptcha = async () => {
    ...
    try {
      ...
      const baseURL = axios.defaults.baseURL || (import.meta.env.VITE_API_BASE ? `${import.meta.env.VITE_API_BASE}` : '');
      
      const config: CaptchaConfig = {
        requestCaptchaDataUrl: `${baseURL}/api/captcha/generate`,
        validCaptchaUrl: `${baseURL}/api/captcha/verify`,
        bindEl: "#captcha-container",
        validSuccess: (res: any, _: any, tac: any) => {
          
          setForm(prev => ({ ...prev, captchaId: res.data.validToken }));

          setShowCaptcha(false);
          tac.destroyWindow();
          performLogin({ ...form, captchaId: res.data.validToken });
        },
    ...
    <DefaultLayout>
      ...
      {/* 版权信息 - 固定在底部，不占据布局空间 */}
               ...
               <p className="text-xs text-gray-400 dark:text-gray-500">
                 Powered by{' '}
                 <a 
                   href="https://github.com/bqlpfy/relay-panel"
                   target="_blank" 
